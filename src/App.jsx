...
function App() {
	...
	const searchForSymbol = async keywords => {
		try {
			const { data } = await axios(
				`https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${keywords}&apikey=${process.env.REACT_APP_API_KEY}`
			);
			if (keywords != "") {
	...
	const fetchGlobalQuote = async () => {
		try {
			const { data } = await axios(
				`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${currentSymbol}&apikey=${process.env.REACT_APP_API_KEY}`
			);
	...
	const getStockData = async (
		...
		try {
			const { data } = await axios(
				`https://www.alphavantage.co/query?function=${timeSeriesUrl}&symbol=${currentSymbol}${interval}&apikey=${process.env.REACT_APP_API_KEY}`
			);
			setCurrentStockData(
